...
class TestProductAttributeMassEdit(TransactionCase):
    ...
    def get_wizard(self):
        return self.env['product.attribute.mass.edit.wizard'].with_context(
            active_ids=self.product_tmpl_ids.ids).create({})
    ...
    def test_without_variant_creation(self):
        ...
        for product in self.product_tmpl_ids:
            ...
            self.assertEqual(
                attribute_line_id.value_ids, self.attribute2_1)
        ...
        for product in self.product_tmpl_ids:
            attribute_line_id = self.product1.attribute_line_ids.filtered(
                lambda l: l.attribute_id == self.attribute2)
            self.assertEqual(
                attribute_line_id.value_ids, self.attribute2_2)
