odoo.define('pos_voucher.pos_voucher', function (require) {
    ...
    models.load_models({
        model: 'pos.voucher',
        fields: ['code', 'type_id', 'partner_id', 'start_date', 'end_date',
        ...
        loaded: function (self, vouchers) {
            ...
            for (var x = 0; x < vouchers.length; x++) {
                ...
                if (self.voucher_by_customer[vouchers[x].partner_id[0]]) {
                    self.voucher_by_customer[vouchers[x].partner_id[0]].push(
                        vouchers[x]);
                } else {
                    self.voucher_by_customer[vouchers[x].partner_id[0]] =
                        [vouchers[x]];
                }
            }
        },
    ...
    models.Order = models.Order.extend({
        export_as_JSON: function () {
            ...
            if (this.voucher_id) {
                json.voucher_id = parseInt(this.voucher_id, 0);
            }
            return json;
        },
